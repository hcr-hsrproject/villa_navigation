...
#define MAP_MAX_X 10.0
#define MAP_MAX_Y 10.0

class Edgeleg_manager_srv
{
public:
  /// constructor
  Edgeleg_manager_srv(ros::NodeHandle nh);

  /// destructor
  virtual ~Edgeleg_manager_srv();
  ...
  void check_IsrobotMoving();

  void resetTargetbasedYolo();


  double getDistance_from_Vec(std::vector<double> origin, double _x, double _y);
  double get_Distance_3d(std::vector<double> pose1,std::vector<double> pose2);
  bool Comparetwopoistions(std::vector<double> pos,std::vector<double> pos2,double criterion);
  bool followingLoop(human_filter::set_target_to_follow::Request &req, human_filter::set_target_to_follow::Response &res);
...
private:
  ...
  double last_global_pose_t;
  std::vector<double> last_global_pose;
