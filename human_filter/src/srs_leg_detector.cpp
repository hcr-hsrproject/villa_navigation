...
// services
#include <human_filter/DetectLegs.h>
...
public:
	...
	//	//laser_sub_(nh_,"scan",10),
		...
		laser_notifier_(laser_sub_,tfl_,fixed_frame,10),
                detected_legs ()
        //        client ("people_decision_making_actions",true) - - pausing no longer needed
                
	{
		...
		tracker_measurements_pub_ = nh_.advertise<people_msgs::PositionMeasurement>("people_tracker_measurements",1);
              ...
              //  human_distance_pub_= nh_.advertise<people_msgs::HS_distance>("HS_distance",10);                //mk
...
// actionlib Resume Call
 bool sendActionLibGoalResume()
   ...
   //          Client client ("people_decision_making_actions",true);
		...
		for(list<Pair*>::iterator i = candidates.begin();i != candidates.end();i++){
			...
			if((*i)->dist_sep_m==0){
				...
				if ((*i)->dist_sep_m<=leg_pair_separation_m){
					Stamped<Point> loc((((*i)->pos1_).operator +=((*i)->pos2_)).operator /=(2.0),scan->header.stamp, scan->header.frame_id);
		...
		try {
			...
			ROS_WARN("TF exception spot 3.");
               ...
               // int y111 = (*loc);
                ...
                ind_x = (loc[0] / dynamic_map.info.resolution + dynamic_map.info.width / 2);  //x index in the ocupancy map
                ind_y = (loc[1] / dynamic_map.info.resolution + dynamic_map.info.height / 2); //y index in the ocupancy map

                indtmp = ind_y*140+ind_x; //printing for checking
                //printf ("map index: %i \n",indtmp );

                //printf ( "map x: %f, map y: %f, index_x: %i, index_y:%i, comb index:%i ",loc[0], loc[1], ind_x , ind_y, ind_y*140+ind_x );
                map_value =  dynamic_map.data [ind_y*140+ind_x];
                //printf ("map_value: %f \n",map_value); 
